...
neon experiment. It automatically conects and starts the recording.
It also sends events to neon to record the timstamp in which each event 
was presented.  
...
def main():
    ...
    stim=True
    while stim:
        user_input=input('Type "start" to begin stimulation,calibrate manually: \n ')
        if start_input==user_input:
            cm.tic()
            print('Starting stimulation...\n initial baseline: ')
            
            device.send_event('baseline_ini_start')
            outlet.push_sample(['baseline_ini_start'])
            
            sleep(INITIAL_BASELINE)

            device.send_event('baseline_ini_end')
    ...
    for im_number, image_stim in enumerate(image_stim_vec):
        ...
        if DRIFT_CORRECTION:
            ...
            device.send_event(images[im_number].name)
    ...
    device.send_event('EndOfExperiment')
    ...
    device.send_event('baseline_end_start')
    outlet.push_sample(['baseline_end_start'])

    sleep(FINAL_BASELINE)

    device.send_event('baseline_end_end')
