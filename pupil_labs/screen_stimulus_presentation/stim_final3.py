...
def main(display_size=(1024,768)):
    ...
    # Maybe reallocate all stimuly in an initial list to optimize stimulation.
    # image_stim=[visual.ImageStim(win, image=im) for im in images]
    ...
    for im in images:
        image_stim = visual.ImageStim(win, image=im)
        image_stim.draw()
        win.flip()
        annotation = p.new_annotation(im.name)
